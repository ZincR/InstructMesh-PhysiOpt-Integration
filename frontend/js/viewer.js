/**
 * Original Three.js Viewer Module
 * Handles the main 3D viewer for original models
 */
...
export class Viewer {
    ...
    init() {
        if (typeof THREE === 'undefined') {
            console.error('[VIEWER] THREE.js is not loaded');
            return;
        }
        ...
        console.log(`[VIEWER] Initialized ${this.containerId} with dimensions:`, width, height);
    }
    ...
    addModel(model) {
        ...
        // Store original textures and colors for segmentation
        ...
        model.traverse((child) => {
            if (child.isMesh) {
                ...
                if (child.material) {
                    if (Array.isArray(child.material)) {
                        child.material.forEach((mat) => {
                            if (mat.map) {
                                this.originalTextures.push({ material: mat, map: mat.map });
                                if (THREE.sRGBEncoding !== undefined) {
                        ...
                        if (child.material.map) {
                            this.originalTextures.push({ material: child.material, map: child.material.map });
                            if (THREE.sRGBEncoding !== undefined) {
                ...
                if (child.geometry && child.geometry.attributes.color) {
                    const originalColors = child.geometry.attributes.color.array.slice();
                    this.originalMeshColors = originalColors;
                }
            }
        });
